...
use openx_core::ContentItem;
use openx_core::ModelClient;
use openx_core::ModelProviderInfo;
use openx_core::Prompt;
use openx_core::ResponseEvent;
use openx_core::ResponseItem;
...
async fn run_stream(sse_body: &str) -> Vec<ResponseEvent> {
    ...
    let openx_home = match TempDir::new() {
        Ok(dir) => dir,
...
fn assert_reasoning(item: &ResponseItem, expected: &str) {
    ...
    } = item
    {
        ...
        for part in parts {
            match part {
                openx_core::ReasoningItemContent::ReasoningText { text }
                | openx_core::ReasoningItemContent::Text { text } => combined.push_str(text),
            }
        }
