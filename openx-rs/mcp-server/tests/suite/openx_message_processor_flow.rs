...
use openx_core::protocol_config_types::ReasoningEffort;
use openx_core::protocol_config_types::ReasoningSummary;
...
async fn test_openx_jsonrpc_conversation_flow() {
    ...
    // Temporary OpenX home with config pointing at the mock server.
    let openx_home = tmp.path().join("openx_home");
    std::fs::create_dir(&openx_home).expect("create codex home dir");
    ...
    create_config_toml(&openx_home, &server.uri()).expect("write config");

    // Start MCP server and initialize.
    let mut mcp = McpProcess::new(&openx_home).await.expect("spawn mcp");
    ...
    let send_user_id = mcp
        .send_send_user_message_request(SendUserMessageParams {
            conversation_id,
            items: vec![openx_protocol::mcp_protocol::InputItem::Text {
                text: "text".to_string(),
            }],
        })
        .await
...
async fn test_send_user_turn_changes_approval_policy_behavior() {
    ...
    mcp.send_response(
        request.id,
        serde_json::json!({ "decision": openx_core::protocol::ReviewDecision::Approved }),
    )
    .await
