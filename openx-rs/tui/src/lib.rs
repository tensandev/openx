...
// The standalone `openx-tui` binary prints a short help message before the
// alternate‑screen mode starts; that file opts‑out locally via `allow`.
#![deny(clippy::print_stdout, clippy::print_stderr)]
#![deny(clippy::disallowed_methods)]
use app::App;
use openx_core::AuthManager;
use openx_core::BUILT_IN_OSS_MODEL_PROVIDER_ID;
...
use openx_core::config::find_openx_home;
use openx_core::config::load_config_as_toml_with_cli_overrides;
use openx_core::protocol::AskForApproval;
use openx_core::protocol::SandboxPolicy;
use openx_ollama::DEFAULT_OSS_MODEL;
...
pub async fn run_main(
...
) -> std::io::Result<openx_core::protocol::TokenUsage> {
    ...
    let overrides_cli = openx_common::CliConfigOverrides { raw_overrides };
    ...
    #[allow(clippy::print_stderr)]
    let config_toml = {
        let openx_home = match find_openx_home() {
            Ok(openx_home) => openx_home,
            Err(err) => {
        ...
        match load_config_as_toml_with_cli_overrides(&openx_home, cli_kv_overrides) {
    ...
    let log_dir = openx_core::config::log_dir(&config)?;
    ...
    // OpenX CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("openx-tui.log"))?;
    ...
    let env_filter = || {
        EnvFilter::try_from_default_env()
            .unwrap_or_else(|_| EnvFilter::new("openx_core=info,openx_tui=info"))
    };
    ...
    if cli.oss {
        openx_ollama::ensure_oss_ready(&config)
            .await
...
async fn run_ratatui_app(
...
) -> color_eyre::Result<openx_core::protocol::TokenUsage> {
    ...
    let resume_selection = if r#continue {
        match RolloutRecorder::list_conversations(&config.openx_home, 1, None).await {
            Ok(page) => page
                .items
                .first()
        ...
        match resume_picker::run_resume_picker(&mut tui, &config.openx_home).await? {
            resume_picker::ResumeSelection::Exit => {
                restore();
                session_log::log_session_end();
                return Ok(openx_core::protocol::TokenUsage::default());
            }
            other => other,
        }
    } else {
...
fn get_login_status(config: &Config) -> LoginStatus {
    if config.model_provider.requires_openai_auth {
        ...
        let openx_home = config.openx_home.clone();
