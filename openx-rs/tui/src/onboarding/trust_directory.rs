...
use openx_core::config::set_project_trusted;
use openx_core::git_info::resolve_root_git_project_for_trust;
...
impl WidgetRef for &TrustDirectoryWidget {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        if self.is_git_repo {
            lines.push(
                "  Since this folder is version controlled, you may wish to allow OpenX".into(),
            );
        ...
        if self.is_git_repo {
            lines.push(create_option(
                ...
                "Yes, allow OpenX to work in this folder without asking for approval",
            ));
            lines.push(create_option(
                1,
            ...
            lines.push(create_option(
                0,
                TrustDirectorySelection::Trust,
                "Allow OpenX to work in this folder without asking for approval",
            ));
            lines.push(create_option(
                1,
                TrustDirectorySelection::DontTrust,
...
impl TrustDirectoryWidget {
    fn handle_trust(&mut self) {
        ...
        if let Err(e) = set_project_trusted(&self.openx_home, &target) {
