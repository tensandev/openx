...
use openx_protocol::mcp_protocol::InputItem;
...
async fn test_send_message_success() {
    // Spin up a mock completions server that immediately ends the OpenX turn.
    // Two OpenX turns hit the mock model (session start + send-user-message). Provide two SSE responses.
    let responses = vec![
    ...
    // Create a temporary OpenX home with config pointing at the mock server.
    let openx_home = TempDir::new().expect("create temp dir");
    create_config_toml(openx_home.path(), &server.uri()).expect("write config.toml");

    // Start MCP server process and initialize.
    let mut mcp = McpProcess::new(openx_home.path())
        .await
...
async fn test_send_message_session_not_found() {
    // Start MCP without creating a OpenX session
    let openx_home = TempDir::new().expect("tempdir");
    let mut mcp = McpProcess::new(openx_home.path()).await.expect("spawn");
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize())
        .await
        .expect("timeout")
        .expect("init");
