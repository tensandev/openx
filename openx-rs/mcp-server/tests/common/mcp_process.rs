...
use openx_protocol::mcp_protocol::CancelLoginChatGptParams;
use openx_protocol::mcp_protocol::GetAuthStatusParams;
...
impl McpProcess {
    pub async fn new(openx_home: &Path) -> anyhow::Result<Self> {
        // Use assert_cmd to locate the binary path and then switch to tokio::process::Command
        let std_cmd = StdCommand::cargo_bin("openx-mcp-server")
            .context("should find binary for openx-mcp-server")?;
        ...
        cmd.env("OPENX_HOME", openx_home);
        cmd.env("RUST_LOG", "debug");

        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("openx-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn send_openx_tool_call(
        &mut self,
        params: CodexToolCallParam,
    ) -> anyhow::Result<i64> {
        let openx_tool_call_params = CallToolRequestParams {
            name: "codex".to_string(),
            arguments: Some(serde_json::to_value(params)?),
        };
        self.send_request(
            mcp_types::CallToolRequest::METHOD,
            Some(serde_json::to_value(openx_tool_call_params)?),
        )
        .await
    }
