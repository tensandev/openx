...
use openx_core::ModelProviderInfo;
use openx_core::NewConversation;
use openx_core::OpenXAuth;
use openx_core::WireApi;
use openx_core::built_in_model_providers;
...
/// Writes an `auth.json` into the provided `openx_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        openx_home.path().join("auth.json"),
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = openx_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![openx_protocol::models::ContentItem::InputText {
    ...
    let prior_system = openx_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![openx_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = openx_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure OpenX to resume from our file
    let model_provider = ModelProviderInfo {
        base_url: Some(format!("{}/v1", server.uri())),
        ..built_in_model_providers()["openai"].clone()
    };
    let openx_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&openx_home);
    ...
    let conversation_manager =
        ConversationManager::with_auth(OpenXAuth::from_api_key("Test API Key"));
    let NewConversation {
        conversation: codex,
        session_configured,
        ..
...
async fn includes_session_id_and_model_headers_in_request() {
    ...
    assert_eq!(request_originator.to_str().unwrap(), "openx_cli_rs");
    assert_eq!(
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut config = load_default_config_for_test(&openx_home);
    config.model_provider = model_provider;
    let conversation_manager = ConversationManager::with_auth(create_dummy_openx_auth());
    let NewConversation {
        conversation: codex,
        conversation_id,
        session_configured: _,
...
async fn prefers_chatgpt_token_when_config_prefers_chatgpt() {
    ...
    let auth_manager = match OpenXAuth::from_openx_home(
        openx_home.path(),
        config.preferred_auth_method,
        &config.responses_originator_header,
    ) {
        Ok(Some(auth)) => openx_core::AuthManager::from_auth_for_testing(auth),
        Ok(None) => panic!("No OpenXAuth found in openx_home"),
        Err(e) => panic!("Failed to load OpenXAuth: {e}"),
    };
...
fn create_dummy_openx_auth() -> OpenXAuth {
    OpenXAuth::create_dummy_chatgpt_auth_for_testing()
}
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under OpenX sandbox network restrictions (mirrors other tests).
    if std::env::var(CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a OpenX sandbox."
        );
