...
impl From<ParsedCommand> for openx_protocol::parse_command::ParsedCommand {
    fn from(v: ParsedCommand) -> Self {
        use openx_protocol::parse_command::ParsedCommand as P;
        match v {
...
/// The easiest way to iterate is to add unit tests and have OpenX fix the implementation.
...
/// Tests are at the top to encourage using TDD + OpenX to fix the implementation.
mod tests {
    use super::*;
    ...
    fn supports_cd_and_rg_files() {
        assert_parsed(
            &shlex_split_safe("cd openx-rs && rg --files"),
            vec![ParsedCommand::Search {
                cmd: "rg --files".to_string(),
                query: None,
                path: None,
