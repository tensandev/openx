...
use openx_core::ConversationItem;
use openx_core::ConversationsPage;
use openx_core::Cursor;
...
pub async fn run_resume_picker(tui: &mut Tui, openx_home: &Path) -> Result<ResumeSelection> {
    let alt = AltScreenGuard::enter(tui);
    let mut state = PickerState::new(openx_home.to_path_buf(), alt.tui.frame_requester());
...
impl PickerState {
    fn new(openx_home: PathBuf, requester: FrameRequester) -> Self {
        Self {
            openx_home,
            requester,
            pagination: Pagination {
    ...
    async fn load_page(&mut self, anchor: Option<&Cursor>) -> Result<()> {
        let page = RolloutRecorder::list_conversations(&self.openx_home, PAGE_SIZE, anchor).await?;
