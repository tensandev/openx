...
use openx_core::AuthManager;
use openx_core::ConversationManager;
use openx_core::Cursor as RolloutCursor;
use openx_core::NewConversation;
use openx_core::OpenXConversation;
use openx_core::RolloutRecorder;
use openx_core::auth::CLIENT_ID;
use openx_core::config::Config;
use openx_core::config::ConfigOverrides;
use openx_core::config::ConfigToml;
use openx_core::config::load_config_as_toml;
use openx_core::exec::ExecParams;
use openx_core::exec_env::create_env;
use openx_core::get_platform_sandbox;
use openx_core::git_info::git_diff_to_remote;
use openx_core::protocol::ApplyPatchApprovalRequestEvent;
use openx_core::protocol::Event;
use openx_core::protocol::EventMsg;
use openx_core::protocol::ExecApprovalRequestEvent;
use openx_core::protocol::InputItem as CoreInputItem;
use openx_core::protocol::Op;
use openx_core::protocol::ReviewDecision;
use openx_login::ServerOptions as LoginServerOptions;
use openx_login::ShutdownHandle;
use openx_login::run_login_server;
use openx_protocol::mcp_protocol::APPLY_PATCH_APPROVAL_METHOD;
use openx_protocol::mcp_protocol::AddConversationListenerParams;
use openx_protocol::mcp_protocol::AddConversationSubscriptionResponse;
use openx_protocol::mcp_protocol::ApplyPatchApprovalParams;
use openx_protocol::mcp_protocol::ApplyPatchApprovalResponse;
use openx_protocol::mcp_protocol::AuthMode;
use openx_protocol::mcp_protocol::AuthStatusChangeNotification;
use openx_protocol::mcp_protocol::ClientRequest;
use openx_protocol::mcp_protocol::ConversationId;
use openx_protocol::mcp_protocol::ConversationSummary;
use openx_protocol::mcp_protocol::EXEC_COMMAND_APPROVAL_METHOD;
use openx_protocol::mcp_protocol::ExecArbitraryCommandResponse;
use openx_protocol::mcp_protocol::ExecCommandApprovalParams;
use openx_protocol::mcp_protocol::ExecCommandApprovalResponse;
use openx_protocol::mcp_protocol::ExecOneOffCommandParams;
use openx_protocol::mcp_protocol::GetUserSavedConfigResponse;
use openx_protocol::mcp_protocol::GitDiffToRemoteResponse;
use openx_protocol::mcp_protocol::InputItem as WireInputItem;
use openx_protocol::mcp_protocol::InterruptConversationParams;
use openx_protocol::mcp_protocol::InterruptConversationResponse;
use openx_protocol::mcp_protocol::ListConversationsParams;
use openx_protocol::mcp_protocol::ListConversationsResponse;
use openx_protocol::mcp_protocol::LoginChatGptCompleteNotification;
use openx_protocol::mcp_protocol::LoginChatGptResponse;
use openx_protocol::mcp_protocol::NewConversationParams;
use openx_protocol::mcp_protocol::NewConversationResponse;
use openx_protocol::mcp_protocol::RemoveConversationListenerParams;
use openx_protocol::mcp_protocol::RemoveConversationSubscriptionResponse;
use openx_protocol::mcp_protocol::ResumeConversationParams;
use openx_protocol::mcp_protocol::SendUserMessageParams;
use openx_protocol::mcp_protocol::SendUserMessageResponse;
use openx_protocol::mcp_protocol::SendUserTurnParams;
use openx_protocol::mcp_protocol::SendUserTurnResponse;
use openx_protocol::mcp_protocol::ServerNotification;
use openx_protocol::mcp_protocol::UserSavedConfig;
...
/// Handles JSON-RPC messages for OpenX conversations.
...
impl CodexMessageProcessor {
    ...
    async fn cancel_login_chatgpt(&mut self, request_id: RequestId, login_id: Uuid) {
        ...
        if guard.as_ref().map(|l| l.login_id) == Some(login_id) {
            ...
            self.outgoing
                .send_response(
                    request_id,
                    openx_protocol::mcp_protocol::CancelLoginChatGptResponse {},
                )
                .await;
        } else {
            drop(guard);
    ...
    async fn logout_chatgpt(&mut self, request_id: RequestId) {
        ...
        self.outgoing
            .send_response(
                request_id,
                openx_protocol::mcp_protocol::LogoutChatGptResponse {},
            )
            .await;
    ...
    async fn get_auth_status(
        ...
        params: openx_protocol::mcp_protocol::GetAuthStatusParams,
    ) {
        ...
        let response = match self.auth_manager.auth() {
            Some(auth) => {
                ...
                openx_protocol::mcp_protocol::GetAuthStatusResponse {
                    auth_method: reported_auth_method,
                    preferred_auth_method,
                    auth_token: token_opt,
                }
            }
            None => openx_protocol::mcp_protocol::GetAuthStatusResponse {
                auth_method: None,
                preferred_auth_method,
                auth_token: None,
            },
    ...
    async fn get_user_saved_config(&self, request_id: RequestId) {
        let toml_value = match load_config_as_toml(&self.config.openx_home) {
            Ok(val) => val,
            Err(err) => {
                let error = JSONRPCErrorError {
                    code: INTERNAL_ERROR_CODE,
    ...
    async fn exec_one_off_command(&self, request_id: RequestId, params: ExecOneOffCommandParams) {
        ...
        let sandbox_type = match &effective_policy {
            openx_core::protocol::SandboxPolicy::DangerFullAccess => {
                openx_core::exec::SandboxType::None
            }
            _ => get_platform_sandbox().unwrap_or(openx_core::exec::SandboxType::None),
        };
        tracing::debug!("Sandbox type: {sandbox_type:?}");
        let openx_linux_sandbox_exe = self.config.openx_linux_sandbox_exe.clone();
        let outgoing = self.outgoing.clone();
        let req_id = request_id;

        tokio::spawn(async move {
            match openx_core::exec::process_exec_tool_call(
                exec_params,
                sandbox_type,
                &effective_policy,
                &openx_linux_sandbox_exe,
    ...
    async fn process_new_conversation(&self, request_id: RequestId, params: NewConversationParams) {
        let config = match derive_config_from_params(params, self.openx_linux_sandbox_exe.clone()) {
            Ok(config) => config,
            Err(err) => {
                let error = JSONRPCErrorError {
                    code: INVALID_REQUEST_ERROR_CODE,
    ...
    async fn handle_resume_conversation(
        ...
        let config = match params.overrides {
            Some(overrides) => {
                derive_config_from_params(overrides, self.openx_linux_sandbox_exe.clone())
            }
            ...
            .await
        {
            Ok(NewConversation {
                ...
                let event = openx_core::protocol::Event {
                    id: "".to_string(),
                    msg: openx_core::protocol::EventMsg::SessionConfigured(
                ...
                let response = openx_protocol::mcp_protocol::ResumeConversationResponse {
                    conversation_id: ConversationId(conversation_id),
...
async fn apply_bespoke_event_handling(
    ...
    conversation: Arc<OpenXConversation>,
    outgoing: Arc<OutgoingMessageSender>,
    pending_interrupts: Arc<Mutex<HashMap<Uuid, Vec<RequestId>>>>,
) {
...
fn find_first_user_text(head: &[serde_json::Value]) -> Option<String> {
    use openx_core::protocol::InputMessageKind;
    for v in head.iter() {
