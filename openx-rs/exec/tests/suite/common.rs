...
/// Helper function to run an E2E test of a openx-exec call. Starts a wiremock
/// server, and returns the response_streams in order for each api call. Runs
/// the openx-exec command with the wiremock server as the model server.
pub(crate) async fn run_e2e_exec_test(cwd: &Path, response_streams: Vec<String>) {
    ...
    let uri = server.uri();
    Command::cargo_bin("openx-exec")
        .context("should find binary for openx-exec")
        .expect("should find binary for openx-exec")
        .current_dir(cwd.clone())
        .env("OPENX_HOME", cwd.clone())
