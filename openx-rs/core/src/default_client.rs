pub const DEFAULT_ORIGINATOR: &str = "openx_cli_rs";

pub fn get_openx_user_agent(originator: Option<&str>) -> String {
...
pub fn create_client(originator: &str) -> reqwest::Client {
    ...
    let ua = get_openx_user_agent(Some(originator));
...
mod tests {
    ...
    #[test]
    fn test_get_openx_user_agent() {
        let user_agent = get_openx_user_agent(None);
        assert!(user_agent.starts_with("openx_cli_rs/"));
    }
    ...
    async fn test_create_client_sets_default_headers() {
        ...
        // User-Agent matches the computed OpenX UA for that originator
        let expected_ua = get_openx_user_agent(Some(originator));
    ...
    fn test_macos() {
        ...
        let user_agent = get_openx_user_agent(None);
        let re = Regex::new(
            r"^openx_cli_rs/\d+\.\d+\.\d+ \(Mac OS \d+\.\d+\.\d+; (x86_64|arm64)\) (\S+)$",
        )
        .unwrap();
