...
use openx_core::protocol::ConversationHistoryResponseEvent;
...
async fn fork_conversation_twice_drops_to_first_message() {
    ...
    // Configure OpenX to use the mock server.
    ...
    let conversation_manager = ConversationManager::with_auth(OpenXAuth::from_api_key("dummy"));
    let NewConversation {
        conversation: codex,
        ..
    ...
    openx_fork1.submit(Op::GetHistory).await.unwrap();
    let fork1_history = wait_for_event(&openx_fork1, |ev| {
    ...
    openx_fork2.submit(Op::GetHistory).await.unwrap();
    let fork2_history = wait_for_event(&openx_fork2, |ev| {
        matches!(ev, EventMsg::ConversationHistory(_))
    })
    .await;
    assert!(matches!(
