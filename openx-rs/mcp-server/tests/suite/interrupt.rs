...
async fn shell_command_interruption() -> anyhow::Result<()> {
    ...
    .await;
    create_config_toml(&openx_home, server.uri())?;

    // Start MCP server and initialize.
    let mut mcp = McpProcess::new(&openx_home).await?;
...
fn create_config_toml(openx_home: &Path, server_uri: String) -> std::io::Result<()> {
