...
use crate::openx::INITIAL_SUBMIT_ID;
use crate::openx::OpenX;
use crate::openx::OpenXSpawnOk;
use crate::openx_conversation::OpenXConversation;
...
/// Represents a newly created OpenX conversation, including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewConversation {
    pub conversation_id: Uuid,
    pub conversation: Arc<OpenXConversation>,
...
pub struct ConversationManager {
    conversations: Arc<RwLock<HashMap<Uuid, Arc<OpenXConversation>>>>,
...
impl ConversationManager {
    ...
    /// Construct with a dummy AuthManager containing the provided OpenXAuth.
    /// Used for integration tests: should not be used by ordinary business logic.
    pub fn with_auth(auth: OpenXAuth) -> Self {
    ...
    async fn spawn_conversation(
        ...
        if let Some(resume_path) = config.experimental_resume.as_ref() {
            ...
            } = OpenX::spawn(config, auth_manager, initial_history).await?;
            self.finalize_spawn(codex, conversation_id).await
        } else {
            let OpenXSpawnOk {
                codex,
                session_id: conversation_id,
            } = { OpenX::spawn(config, auth_manager, InitialHistory::New).await? };
            self.finalize_spawn(codex, conversation_id).await
        }
    }
    ...
    async fn finalize_spawn(
        ...
        let conversation = Arc::new(OpenXConversation::new(codex));
        self.conversations
            .write()
            .await
    ...
    pub async fn get_conversation(
        &self,
        conversation_id: Uuid,
    ) -> CodexResult<Arc<OpenXConversation>> {
    ...
    pub async fn fork_conversation(
        ...
        let OpenXSpawnOk {
            codex,
            session_id: conversation_id,
        } = OpenX::spawn(config, auth_manager, history).await?;
...
mod tests {
    ...
    use openx_protocol::models::ReasoningItemReasoningSummary;
    use openx_protocol::models::ResponseItem;
