...
use openx_core::default_client::create_client;

use crate::version::OPENX_CLI_VERSION;
...
pub fn get_upgrade_version(config: &Config) -> Option<String> {
    ...
    info.and_then(|info| {
        if is_newer(&info.latest_version, OPENX_CLI_VERSION).unwrap_or(false) {
...
fn version_filepath(config: &Config) -> PathBuf {
    config.openx_home.join(VERSION_FILENAME)
}
