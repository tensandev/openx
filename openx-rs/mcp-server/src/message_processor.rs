...
use crate::openx_message_processor::CodexMessageProcessor;
use crate::openx_tool_config::CodexToolCallParam;
use crate::openx_tool_config::CodexToolCallReplyParam;
use crate::openx_tool_config::create_tool_for_openx_tool_call_param;
use crate::openx_tool_config::create_tool_for_openx_tool_call_reply_param;
...
use openx_core::protocol::Submission;
use serde_json::json;
use std::sync::Arc;
...
pub(crate) struct MessageProcessor {
    openx_message_processor: CodexMessageProcessor,
    outgoing: Arc<OutgoingMessageSender>,
    initialized: bool,
    openx_linux_sandbox_exe: Option<PathBuf>,
    conversation_manager: Arc<ConversationManager>,
    running_requests_id_to_openx_uuid: Arc<Mutex<HashMap<RequestId, Uuid>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) fn new(
        ...
        let openx_message_processor = CodexMessageProcessor::new(
            auth_manager,
            conversation_manager.clone(),
            outgoing.clone(),
            openx_linux_sandbox_exe.clone(),
            config,
        );
        Self {
            openx_message_processor,
            outgoing,
            initialized: false,
            openx_linux_sandbox_exe,
            conversation_manager,
            running_requests_id_to_openx_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }

    pub(crate) async fn process_request(&mut self, request: JSONRPCRequest) {
        if let Ok(request_json) = serde_json::to_value(request.clone())
            && let Ok(openx_request) = serde_json::from_value::<ClientRequest>(request_json)
        {
            // If the request is a OpenX request, handle it with the OpenX
            // message processor.
            self.openx_message_processor
                .process_request(openx_request)
                .await;
            return;
        }
    ...
    async fn handle_initialize(
        ...
        let result = mcp_types::InitializeResult {
            ...
            server_info: mcp_types::Implementation {
                name: "openx-mcp-server".to_string(),
                version: env!("CARGO_PKG_VERSION").to_string(),
                title: Some("OpenX".to_string()),
            },
        };
    ...
    async fn handle_list_tools(
        ...
        let result = ListToolsResult {
            tools: vec![
                create_tool_for_openx_tool_call_param(),
                create_tool_for_openx_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(
        ...
        match name.as_str() {
            ...
            "openx-reply" => {
                self.handle_tool_call_openx_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_codex(&self, id: RequestId, arguments: Option<serde_json::Value>) {
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<CodexToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.openx_linux_sandbox_exe.clone()) {
                    Ok(cfg) => cfg,
                    Err(e) => {
                        let result = CallToolResult {
                            content: vec![ContentBlock::TextContent(TextContent {
                                ...
                                text: format!(
                                    "Failed to load OpenX configuration from overrides: {e}"
                                ),
                                annotations: None,
                            })],
                            is_error: Some(true),
                ...
                Err(e) => {
                    let result = CallToolResult {
                        content: vec![ContentBlock::TextContent(TextContent {
                            r#type: "text".to_owned(),
                            text: format!("Failed to parse configuration for OpenX tool: {e}"),
                            annotations: None,
                        })],
                        is_error: Some(true),
                        structured_content: None,
        ...
        let running_requests_id_to_openx_uuid = self.running_requests_id_to_openx_uuid.clone();

        // Spawn an async task to handle the OpenX session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the OpenX session and stream events back to the client.
            crate::openx_tool_runner::run_openx_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                conversation_manager,
                running_requests_id_to_openx_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_openx_session_reply(
        &self,
        ...
        let CodexToolCallReplyParam { session_id, prompt } = match arguments {
            Some(json_val) => match serde_json::from_value::<CodexToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse OpenX tool call reply parameters: {e}");
                    let result = CallToolResult {
                        content: vec![ContentBlock::TextContent(TextContent {
                            r#type: "text".to_owned(),
                            text: format!("Failed to parse configuration for OpenX tool: {e}"),
            ...
            None => {
                tracing::error!(
                    "Missing arguments for openx-reply tool-call; the `session_id` and `prompt` fields are required."
                );
                let result = CallToolResult {
                    content: vec![ContentBlock::TextContent(TextContent {
                        r#type: "text".to_owned(),
                        text: "Missing arguments for openx-reply tool-call; the `session_id` and `prompt` fields are required.".to_owned(),
                        annotations: None,
                    })],
                    is_error: Some(true),
                    structured_content: None,
        ...
        tokio::spawn({
            ...
            let running_requests_id_to_openx_uuid = running_requests_id_to_openx_uuid.clone();

            async move {
                crate::openx_tool_runner::run_openx_tool_session_reply(
                    codex,
                    outgoing,
                    request_id,
                    prompt,
    ...
    async fn handle_cancelled_notification(
        ...
        let session_id = {
            let map_guard = self.running_requests_id_to_openx_uuid.lock().await;
        ...
        // Obtain the OpenX conversation from the server.
        let openx_arc = match self.conversation_manager.get_conversation(session_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for session_id: {session_id}");
                return;
        ...
        // Submit interrupt to OpenX.
        let err = openx_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: openx_core::protocol::Op::Interrupt,
            })
            .await;
        if let Err(e) = err {
            tracing::error!("Failed to submit interrupt to OpenX: {e}");
            return;
        }
        // unregister the id so we don't keep it in the map
        self.running_requests_id_to_openx_uuid
            .lock()
            .await
            .remove(&request_id);
    }
