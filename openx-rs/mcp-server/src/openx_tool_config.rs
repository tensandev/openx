...
pub struct CodexToolCallParam {
    /// The *initial user prompt* to start the OpenX conversation.
    pub prompt: String,
...
/// Builds a `Tool` definition (JSON schema etc.) for the OpenX tool-call.
pub(crate) fn create_tool_for_openx_tool_call_param() -> Tool {
    ...
    let schema_value =
        serde_json::to_value(&schema).expect("OpenX tool schema should serialise to JSON");
    ...
    Tool {
        ...
        output_schema: None,
        description: Some(
            "Run a OpenX session. Accepts configuration parameters matching the OpenX Config struct.".to_string(),
        ),
        annotations: None,
    }
}
...
impl CodexToolCallParam {
    /// Returns the initial user prompt to start the OpenX conversation and the
    /// effective Config object generated from the supplied parameters.
    pub fn into_config(
        self,
        openx_linux_sandbox_exe: Option<PathBuf>,
    ) -> std::io::Result<(String, openx_core::config::Config)> {
        let Self {
            prompt,
            model,
            profile,
        ...
        // Build the `ConfigOverrides` recognized by openx-core.
        let overrides = openx_core::config::ConfigOverrides {
            model,
        ...
        let cfg = openx_core::config::Config::load_with_cli_overrides(cli_overrides, overrides)?;
...
pub struct CodexToolCallReplyParam {
    ...
    /// The *next user prompt* to continue the OpenX conversation.
    pub prompt: String,
}

/// Builds a `Tool` definition for the `openx-reply` tool-call.
pub(crate) fn create_tool_for_openx_tool_call_reply_param() -> Tool {
    let schema = SchemaSettings::draft2019_09()
        .with(|s| {
            s.inline_subschemas = true;
            s.option_add_null_type = false;
    ...
    #[expect(clippy::expect_used)]
    let schema_value =
        serde_json::to_value(&schema).expect("OpenX reply tool schema should serialise to JSON");
    ...
    Tool {
        name: "openx-reply".to_string(),
        title: Some("OpenX Reply".to_string()),
        input_schema: tool_input_schema,
        output_schema: None,
        description: Some(
            "Continue a OpenX session by providing the session id and prompt.".to_string(),
        ),
        annotations: None,
    }
}
...
mod tests {
    ...
    #[test]
    fn verify_openx_tool_json_schema() {
        let tool = create_tool_for_openx_tool_call_param();
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
          "name": "codex",
          "title": "OpenX",
          "description": "Run a OpenX session. Accepts configuration parameters matching the OpenX Config struct.",
          "inputSchema": {
            "type": "object",
            "properties": {
              "approval-policy": {
              ...
              "config": {
                "description": "Individual config settings that will override what is in OPENX_HOME/config.toml.",
              ...
              "prompt": {
                "description": "The *initial user prompt* to start the OpenX conversation.",
                "type": "string"
              },
              "base-instructions": {
    ...
    #[test]
    fn verify_openx_tool_reply_json_schema() {
        let tool = create_tool_for_openx_tool_call_reply_param();
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
          "description": "Continue a OpenX session by providing the session id and prompt.",
          "inputSchema": {
            "properties": {
              "prompt": {
                "description": "The *next user prompt* to continue the OpenX conversation.",
                "type": "string"
              },
              "sessionId": {
