...
async fn exec_cli_applies_experimental_instructions_file() {
    ...
    // OpenX to use the Responses API with the dummy env var.
    let provider_override = format!(
...
/// 2. Configures codex to read from this fixture via OPENX_RS_SSE_FIXTURE env var
...
async fn responses_api_stream_cli() {
    ...
    cmd.env("OPENX_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("OPENX_RS_SSE_FIXTURE", fixture)
...
async fn integration_creates_and_checks_session_file() {
    ...
    cmd2.arg("run")
        ...
        .arg(env!("CARGO_MANIFEST_DIR"))
        .arg(&prompt2);
    cmd2.env("OPENX_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("OPENX_RS_SSE_FIXTURE", &fixture)
        .env("OPENAI_BASE_URL", "http://unused.local");

    let output2 = cmd2.output().unwrap();
    assert!(output2.status.success(), "resume openx-cli run failed");
...
async fn integration_git_info_unit_test() {
    ...
    let git_info = openx_core::git_info::collect_git_info(&git_repo).await;
    ...
    let deserialized: openx_core::git_info::GitInfo = serde_json::from_str(&serialized).unwrap();
