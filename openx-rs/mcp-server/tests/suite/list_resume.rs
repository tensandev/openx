...
use openx_protocol::mcp_protocol::NewConversationParams; // reused for overrides shape
use openx_protocol::mcp_protocol::ResumeConversationParams;
use openx_protocol::mcp_protocol::ResumeConversationResponse;
...
async fn test_list_and_resume_conversations() {
    // Prepare a temporary OPENX_HOME with a few fake rollout files.
...
fn create_fake_rollout(openx_home: &Path, filename_ts: &str, meta_rfc3339: &str, preview: &str) {
    let uuid = Uuid::new_v4();
    // sessions/YYYY/MM/DD/ derived from filename_ts (YYYY-MM-DDThh-mm-ss)
    let year = &filename_ts[0..4];
    let month = &filename_ts[5..7];
    let day = &filename_ts[8..10];
    let dir = openx_home.join("sessions").join(year).join(month).join(day);
