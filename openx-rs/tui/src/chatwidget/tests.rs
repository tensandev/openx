...
use openx_core::plan_tool::PlanItemArg;
use openx_core::plan_tool::StepStatus;
use openx_core::plan_tool::UpdatePlanArgs;
...
use openx_core::protocol::PatchApplyEndEvent;
use openx_core::protocol::StreamErrorEvent;
use openx_core::protocol::TaskCompleteEvent;
use openx_core::protocol::TaskStartedEvent;
...
fn test_config() -> Config {
    ...
    openx_core::config::Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
...
fn final_answer_without_newline_is_flushed_immediately() {
    ...
    chat.handle_openx_event(Event {
        id: "sub-a".into(),
        msg: EventMsg::AgentMessageDelta(AgentMessageDeltaEvent {
            delta: "Hi! How can I help with openx-rs or anything else today?".into(),
        }),
    });
    ...
    chat.handle_openx_event(Event {
        ...
        msg: EventMsg::AgentMessage(AgentMessageEvent {
            message: "Hi! How can I help with openx-rs or anything else today?".into(),
        }),
    });
    ...
    let found_final = cells.iter().any(|lines| {
        let s = lines
            ...
            .map(|sp| sp.content.clone())
            .collect::<String>();
        s.contains("Hi! How can I help with openx-rs or anything else today?")
    });
    assert!(
        found_final,
...
fn resumed_initial_messages_render_history() {
    ...
    let configured = openx_core::protocol::SessionConfiguredEvent {
        session_id: Uuid::nil(),
        model: "test-model".to_string(),
        history_log_id: 0,
        history_entry_count: 0,
        initial_messages: Some(vec![
            EventMsg::UserMessage(openx_core::protocol::UserMessageEvent {
...
async fn helpers_are_available_and_do_not_panic() {
    ...
    let cfg = test_config();
    let conversation_manager = Arc::new(ConversationManager::with_auth(OpenXAuth::from_api_key(
        "test",
    )));
...
fn make_chatwidget_manual() -> (
    ...
    let bottom = BottomPane::new(BottomPaneParams {
        ...
        enhanced_keys_supported: false,
        placeholder_text: "Ask OpenX to do anything".to_string(),
...
fn begin_exec(chat: &mut ChatWidget, call_id: &str, raw_cmd: &str) {
    ...
    let parsed_cmd: Vec<openx_protocol::parse_command::ParsedCommand> =
        openx_core::parse_command::parse_command(&command)
            .into_iter()
            .map(Into::into)
            .collect();
    chat.handle_openx_event(Event {
...
fn interrupt_exec_marks_failed_snapshot() {
    ...
    chat.handle_openx_event(Event {
        id: "call-int".into(),
        msg: EventMsg::TurnAborted(openx_core::protocol::TurnAbortedEvent {
...
async fn binary_size_transcript_snapshot() {
                ...
                if let Some(payload) = v.get("payload") {
                    ...
                    let ev = match ev {
                        Event {
                            ...
                            let parsed_cmd = openx_core::parse_command::parse_command(&e.command);
                            Event {
                                id: ev.id,
                                msg: EventMsg::ExecCommandBegin(ExecCommandBeginEvent {
    ...
    const MARKER_PREFIX: &str = ">_ You are using OpenAI OpenX in ";
...
//
// Synthesizes a OpenX ExecApprovalRequest event to trigger the approval modal
...
fn approval_modal_exec_snapshot() {
    ...
    chat.config.approval_policy = openx_core::protocol::AskForApproval::OnRequest;
...
fn interrupt_restores_queued_messages_into_composer() {
    ...
    chat.handle_openx_event(Event {
        id: "turn-1".into(),
        msg: EventMsg::TurnAborted(openx_core::protocol::TurnAbortedEvent {
            reason: openx_core::protocol::TurnAbortReason::Interrupted,
        }),
    });
...
fn apply_patch_approval_sends_op_with_submission_id() {
    ...
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::CodexOp(Op::PatchApproval { id, decision }) = app_ev {
            ...
            assert!(matches!(
                decision,
                openx_core::protocol::ReviewDecision::Approved
            ));
            found = true;
            break;
        }
...
fn apply_patch_full_flow_integration_like() {
    ...
    // 3) App forwards to widget.submit_op, which pushes onto openx_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
...
fn chatwidget_exec_and_status_layout_vt100_snapshot() {
    ...
    chat.handle_openx_event(Event {
        ...
        msg: EventMsg::ExecCommandBegin(ExecCommandBeginEvent {
            ...
            cwd: std::env::current_dir().unwrap_or_else(|_| PathBuf::from(".")),
            parsed_cmd: vec![
                openx_core::parse_command::ParsedCommand::Search {
                    query: Some("Change Approved".into()),
                    path: None,
                    cmd: "rg \"Change Approved\"".into(),
                }
                .into(),
                openx_core::parse_command::ParsedCommand::Read {
